<template lang="jade">
  .escalate-card
    .escalate-card-light(v-if="config.isBlink")
      //进教室挂宠物
    img.pet-img(v-if="config.isBlink",:src="petImgSrc?petImgSrc:null")
    .header
      .lesson-name
        | {{config.mainTitle}}
      .date
        ...
        span.time {{dateTime()}}
      //- span.video-name
      //-   | {{config.aiCourseCard.mainTitle}}
      .hand(ref="hand")
        .h-black
        .h-line(ref="handLine")
        .h-circle.icon-hand-circle(ref="cardHand",@click="handClick",@mouseenter="handHover")
    .live-course(:style="{backgroundColor: currentPanelState=='face'?'#ffffff':'#6ad3c6'}")
        ...
        .front-face(key="1",v-if="currentPanelState=='face'")
          .mid
            ...
            template(v-if="config.cardStyle=='pre_video'")
              img.pre-video-img(:src="config.imageInfo&&config.imageInfo.imageUrl?config.imageInfo.imageUrl:null")
            //进教室
            classroom(v-if="config.cardStyle=='classroom'",:classInfo="config.classInfo")
            //flash card
            flash-card(v-if="config.cardStyle=='flash_card'",:cardConfig="config.homeworkInfo")
            //做作业
            homework(v-if="config.cardStyle=='homework'",:cardConfig="config.homeworkInfo")
          //卡片核心按钮
          vk-btn.star-btn(:type="classBtn",:text="config.bottomButton.buttonText",:clickFun="buttonClick")
          .progress
            vk-progress(:pps="pps")
        //背面
...
<style lang="stylus" scoped>
  ...
  .header
    ...
    width: 100%
    height: 175px
    // background: url(../../../img/index/home_card_mayjor_top@2x.png) no-repeat top
