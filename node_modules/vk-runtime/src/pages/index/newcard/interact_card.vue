<template lang="jade">
  .interact-card
    .tnteract-card-light(v-if="config.isBlink")
      //进教室挂宠物
    ...
    .live-course(:style="{backgroundColor: currentPanelState=='face'?'#ffffff':'#7fd16f'}")
      transition-group(enter-active-class="animated fadeInDown")
        ...
        .front-face(key="1",v-if="currentPanelState=='face'")
          .mid
            //绘本
            template(v-if="config.cardStyle=='reader'")
              .book-hd
                .book-shadow
                img.reader-book(:src="config.imageInfo.imageUrl",@error="errorImg")
            //课前预习视频
            ...
            template(v-if="config.cardStyle=='homework'")
              ...
              template(v-else)
                homework
          //卡片核心按钮
          vk-btn.star-btn(v-if="config.bottomButton.buttonFunction == 'READ_BOOK'",:type="classBtn",:text="config.bottomButton.buttonText",:clickFun="goBookDetail")
          vk-btn.star-btn(v-else, :type="classBtn",:text="config.bottomButton.buttonText",:clickFun="buttonClick",:alertText="alertText")   
          .progress
            vk-progress(:pps="pps")
        //背面
...
<script>
  ...
  const noImg = require("../../../img/index/pre_bg.png")
  ...
  export default {
    ...
    methods: {
      ...
      // 读绘本
      goBookDetail() {
        let bookId = this.config.bottomButton.buttonExtra.bookId
        let bookCode = this.config.bottomButton.buttonExtra.bookCode
