<template lang="jade">
  .me-index-content
    ...
    .me-index-list.clearfix(v-if="channels.length!=0")
      .label Listening
      .me-index-item
    ...
    .me-index-list.list-below.clearfix(v-if="mcReaderTagsList.length!=0")
      .label.another Reading
      template(v-for="item in mcReaderTagsList")
        .me-index-item
          img(class="iconImg", :src="item.imgUrl", :style="item.status==status['locked']?{opacity:0.5}:{}", @click="item.status==status['locked']?null:gotoLibraryList(item.id)")
          .me-item-lock(v-if="item.status==status['locked']")
            .lockTip(:style="lockTipShow[0]?{opacity:1}:{opacity:0}") {{lockTip}}
            img(class="lockImg", @click="lockClick($event,0)", src="../../img/major_extension/list-lock@3x.png")
          .mii-title-en {{item.name}}
            img(class="icon-new animated zoomIn infinite", v-if="item.status==status['new']", src="../../img/major_extension/icon-new.png")
          .mii-title-cn {{item.chName}}
</template>
<script>
...
export default {
  data: function () {
    return {
      ...
      mcReaderTagsList: [],
      getMcReaderTagsUrl: '/rest/learninggw/api/pc/material/getReaderByStudentID',
      channels: []
    }
  },
  components: {
  ...
  mounted() {
    this.getList()
    this.getMcReaderTags()
  },
  methods: {
    gotoLibraryList(id){
      this.$router.push({ path: '/dgl/library/' + id})
    },
    //去往详情页面，拓展视频
    goToDetail(index) {
    ...
    getMcReaderTags() {
      axios.get(this.getMcReaderTagsUrl, {
        params: {
          studentId: cookies.get("studentId")
        }
      })
        .then((res) => {
          this.mcReaderTagsList = res.data.data
        })
        .catch((err) => {
        })
    }
  ...
  .me-index-content
    ...
    .me-index-list
      ...
      .label
        ...
        background-color: rgba(166, 139, 233, 0.1)
        font-size: 18px
        text-align: center
        color: #ae7eff
        line-height: 42px
