<template lang="jade">
  transition(name="extension", enter-active-class="animated fadeInRight", leave-active-class="animated fadeOutRight")
    .pd(@mousewheel="userScrollDown($event)", @DOMMouseScroll="userScrollDown($event)", @touchstart="touchStart($event)", @touchmove="touchMove($event)", @touchend="touchEnd($event)")
      ...
      .pd-container(v-if="detail")
        ...
        //- 主要内容
        .pd-detail-container
          english-speech(:detail="detail", @rule="openRule('join')", @gojoin="goJoin", :partitionId="partitionId")
          .pd-detail-video(:class="{'active':showBox}", @mousewheel.stop="userScrollDown($event, 'pd-detail-video')", @DOMMouseScroll.stop="userScrollDown($event, 'pd-detail-video')", @click="openVideo($event)")
            .pd-detail-bottom
...
<script>
  ...
  import englishSpeech from './components/english_speech_box.vue'
  ...
  export default {
    data() {
      return {
        ...
        whichActivity: 'ENGLISH_SPEECH', // 活动类型
        activityName: '',//活动名称
        activityTheme: '',//活动主题
        activityType: 0 // 活动类型1, 2, 3, 4  --  3 代表  show time 4 代表 21 世纪
      }
    },
    components: {
      pdBack,
    ...
    methods: {
      getDetailByPid() {
        axios.get(`/rest/activity/api/activity/user/activity/getActivityDetailByActivityId/${this.pdId}`)
          .then((res) => {
            this.loading = false
            let result = res.data
      ...
      openVideo(e){
        if((e.target.className == 'video-box') || (e.target.className == 'pd-detail-label')){
          if(this.showBox){
            this.showBox = false
          }else{
            this.showBox = true
      ...
      getWinInfo(){
        axios.get(`/rest/activity/api/activity/user/message/getMessage/${this.pdId}`)
          .then((res) => {
...
<style lang="stylus" scoped>
  ...
  .pd
    background: #382A73 url(../../img/project_demo/speech_bg.png) no-repeat
