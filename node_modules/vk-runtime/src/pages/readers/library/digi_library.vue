<template lang="jade">
  .library-wrapper(v-cloak)
    ...
    .library-list(v-if="!isNoAuth")
      filter-component(@closeUserGuideBUS = "closeUserGuide", @setBookMarkerBUS = "setBookMarker", :levelIds = "levelIds", :subjectIds = "subjectIds", @filterEvent="getBooklistByFilter", :isShowUserGuideSecond="isShowUserGuideSecond", :currentLevelId="currentLevelId")
            ...
            li.library-list-item(:class="[index > 3 ? 'no-margin-bottom' : '']", @click="openBookDetail(item.id, item.borrowStatus)", @mouseenter="bookNameMouseEnter($event)", @mouseleave="bookNameMouseLeave($event)")
              .book-img-contain(@mouseenter="reactMouseEnter($event)", @mouseleave="reactMouseLeave($event)", :class="animateCss")
                ...
                template(v-if="!isAllSubjects")
                  .book-marker.book-marker-no-content-math(v-if="item.tag[1].id == DLType_Math_Id")
                  .book-marker.book-marker-no-content-science(v-if="item.tag[1].id == DLType_Science_Id")
                  .book-marker.book-marker-no-content-social(v-if="item.tag[1].id == DLType_Social_Id")
                template(v-else)
                  .book-marker.book-marker-math(v-if="item.tag[1].id == DLType_Math_Id")
                  .book-marker.book-marker-science(v-if="item.tag[1].id == DLType_Science_Id")
                  .book-marker.book-marker-social(v-if="item.tag[1].id == DLType_Social_Id")
      ...
      .library-list-item
        ...
        .book-img-contain
          ...
          .book-marker
            ...
            bottom: -7px
            left: 15px
            &.book-marker-no-content-math
              width: 28px
              height: 15px
              background-position: -224px -5px
              bottom: 4px
            &.book-marker-no-content-social
              width: 28px
              height: 15px
              background-position: -189px -5px
              bottom: 4px
            &.book-marker-no-content-science
              width: 28px
              height: 15px
...
<script>
  ...
  import filterComponent from "../component/filter.vue"
  ...
  export default {
    ...
    methods: {
      ...
      initData() {
        this.setLevelIds() //获取学生当前level依赖于这个数组
        this.setSubjectIds()
      ...
      setLevelIds() {
        $get('/api/pc/service/digitlib/tags/level', {})
          .then((res) => {
            res.data.data.vkForEach((item, index) => {
              this.levelIds.push(item.id)
            })
            this.getStuCurrentLevel()
          })
          .catch((err) => {
            this.isFullPageLoadingShow = false
      ...
      setSubjectIds() {
        $get('/api/pc/service/digitlib/tags/dl', {})
          .then((res) => {
            res.data.data.vkForEach((item, index) => {
      ...
      getBooklistByFilter(arr) {
        for (var i = 0; i < arr.length; i++) {
          if (arr[i] == 'allSubjects' || arr[i] == 'allLevels') {
            arr.splice(i, 1)
            i--
          }
        }
      ...
      getStuCurrentLevel() {
        $get(`/api/pc/service/digitlib/${cookies.get('studentId')}/currentLevel`)
          .then((res) => {
            if (res.data.code == 200 && res.data.data) {
              let level = res.data.data.serialNumber.match(/\d/)[0]
              this.currentLevelId = this.levelIds[level - 1]
              // this.currentLevelId = 'allLevels'
              this.filterId = this.currentLevelId == 'allLevels' ? '' : this.currentLevelId
              this.$nextTick(() => {
                this.getBookList()
              })
            } else {
      ...
      getBookList() {
        ...
        $get(this.bookListUrl, {
          ...
          .then((res) => {
            ...
            if (!res.data.data.books) this.isFullPageLoadingShow = false
            this.$nextTick(() => {
              for (let i = 0; i < 8; i++) {
