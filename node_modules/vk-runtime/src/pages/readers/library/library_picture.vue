  ...
  .library-wrapper
    ...
    .library-list
      ...
      ul.library-list-body
        template(v-for="(item,index) in journeysBookList")
...
.library-list {
    ...
    .library-list-item {
        ...
        .read-status-end, .read-status-new {
            width: 72px;
            height: 28px;
            line-height: 28px;
            position: absolute;
...
export default {
  ...
  data() {
    return {
      ...
      journeysBookUrl: "/api/pc/service/reader/getJourneysBook",
  ...
  mounted() {
    if (this.$route.path == "/libraryPictrue/journeys") {
      this.title_cn = "Journeys 数字图书馆"
      this.title_ng = "Digital Library of Journeys"
    }
    this.getJourneysBookList()
  },
  methods: {
    closeDetailWindow() {
    ...
    getJourneysBookList() {
      $get(this.journeysBookUrl, {
        studentId: cookies.get("studentId"),
        currentPage: this.page
      })
        .then(res => {
          this.journeysBookList = []
          for (let i = 0; i < 8; i++) {
            if (res.data.data.books[i]) {
              res.data.data.books[i].isdashed = false
              this.journeysBookList.push(res.data.data.books[i])
            } else {
              this.journeysBookList.push({ isdashed: true })
            }
          }
    ...
    openBookDetail(id) {
      sa.track('learning_click', {'click_id': 'pc_learning_digital_library_card_enter_show'})
      this.currentBookDetail = this.journeysBookList[id]
      let level = `Level ${this.currentBookDetail.curriculumSN[0]
        .split("-")[1]
        .substr(1)}`
      let unit = `Unit ${this.currentBookDetail.curriculumSN[0]
        .split("-")[2]
        .substr(1)}`
