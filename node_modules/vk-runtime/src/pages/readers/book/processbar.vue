<template lang="jade">
  .box(v-if="isProcessBarShow")
    .progressbar
      .bar(:class="barColor", ref="bar")
        span
      .label(ref="barLabel")
        .perc {{percent}}%
        span
...
export default {
  props: {
    ...
    barColor: { // bar的主题颜色
      // 可选项: color1，color2, color3, color4
      type: String,
      default: "color5"
    }
  },
  data() {
    return {
      isProcessBarShow: false, // 是否显示加载进度条
      percent: 0, // 已加载的 百分比
      barWidth: 0, // bar 的宽度
      barMaxWidth: 558 // bar 的最大宽度
    }
  },
  created() {},
  mounted() {
  ...
  methods: {},
  watch: {
    // 监听，start的数据变化，由false --> true后，开始周期性计时改变 this.barWidth 的值
    start(val, oldVal) {
      val && (this.isProcessBarShow = true) // 显示processbar
    },
    // 监听，loadedNumber的变化， 从而改变 percent, 及 相关样式的变化
    loadedNumber(val, oldVal) {
      this.percent = Math.ceil(val / this.sumNumber * 100)
      this.barWidth = this.barMaxWidth * this.percent * 0.01 + 'px'
      this.$refs.bar.style.width = this.barWidth
      this.$refs.barLabel.style.left = this.barWidth
    },
    // 监听，stop的数据变化，由false --> true后，开始计时，计时时间后，processbar消失，触发 processComplete事件
    stop(val, oldVal) {
      if (val) {
        setTimeout(() => {
...
.box
  ...
  left: 0
  z-index: 999
  background-color: rgba(23, 2, 62, 0.9)
  .progressbar
    position: relative
    top: 38.2%
    display: block
    ...
    .bar
      ...
      &.color5
        background: rgb(154, 46, 171)
        background: -moz-linear-gradient(top, rgba(154, 46, 171, 1) 0%, rgba(179, 150, 255, 1) 100%)
        background: -webkit-gradient(top, left top, left bottom, color-stop(0%, rgba(154, 46, 171, 1)), color-stop(100%, rgba(179, 150, 255, 1)))
        background: -webkit-linear-gradient(top, rgba(154, 46, 171, 1) 0%, rgba(179, 150, 255, 1) 100%)
        background: -o-linear-gradient(top, rgba(154, 46, 171, 1) 0%, rgba(179, 150, 255, 1) 100%)
        background: -ms-linear-gradient(top, rgba(154, 46, 171, 1) 0%, rgba(179, 150, 255, 1) 100%)
        background: linear-gradient(to bottom, rgba(154, 46, 171, 1) 0%, rgba(179, 150, 255, 1) 100%)
        -webkit-box-shadow: 0px 0px 12px 0px rgba(154, 46, 171, 1), inset 0px 1px 0px 0px rgba(255, 255, 255, 0.45), inset 1px 0px 0px 0px rgba(255, 255, 255, 0.25), inset -1px 0px 0px 0px rgba(255, 255, 255, 0.25)
        box-shadow: 0px 0px 12px 0px rgba(154, 46, 171, 1), inset 0px 1px 0px 0px rgba(255, 255, 255, 0.45), inset 1px 0px 0px 0px rgba(255, 255, 255, 0.25), inset -1px 0px 0px 0px rgba(255, 255, 255, 0.25)
      &:before
        position: absolute
        display: block
        content: ''
