<template lang="jade">
  .book-wrapper(v-cloak)
    ...
    .book-contain
      ...
      .leftbtn(@click="pageTurn('pre')", v-show="isShowLeftBtn && enableAction")
      .rightbtn(@click="pageTurn('next')", v-show="isShowRightBtn && enableAction")
      ...
      .reader-content(v-show="isShowBookContent")
        .first-part(:data-group = "bookPageFirst.group")
          ...
          template(v-for="item in bookPageFirst.audios")
            audio(v-show = "false", :src="item.url", controls, id="audio-1", class="parentaduio")
          i.book-page {{bookPageFirst.sequence}}
          template(v-if="bookPageFirst.isAudio")
            span.sound-control-btn(data-page="one", @click.stop="audioControlOnPageCorner($event)", v-if="!status.isMute", :class="{disabled: !enableAction}")
        ...
        .turn-page(v-for="(item,index) in bookPageList")
          .page-show(:data-group = "item.pageOne.group")
            ...
            audio(v-if = "item.pageOne.audios.length > 0", :src="item.pageOne.audios[0].url" controls, :id="'audio-' + item.pageOne.sequence", class="parentaduio")
            i.book-page.turn-book-page-right.page-right {{item.pageOne.sequence}}
            template(v-if="item.pageOne.isAudio")
              span.sound-control-btn.replay-btn-right(data-page="two", @click.stop="audioControlOnPageCorner($event)", v-if="!status.isMute", :class="{disabled: !enableAction}")
                img.wave-1(src="../../../img/library/voice_wave_1.png")
                img.wave-2(src="../../../img/library/voice_wave_2.png")
                img.wave-3(src="../../../img/library/voice_wave_3.png")
          ...
          .page-hide(:data-group = "item.pageTwo.group")
            ...
            audio(v-if = "item.pageTwo.audios.length > 0", :src="item.pageTwo.audios[0].url" controls, :id="'audio-'+ item.pageTwo.sequence", class="parentaduio")
...
<script>
  ...
  import bookRecrdAction from '../component/book_record_action_v2.vue'
  ...
  export default {
    ...
    data() {
      return {
        ...
        dudSummaryId:0,
        // jcy add 
        enableAction: true // 当处在 录音播放 或者 录制音频播放状态时，控制按钮的显示隐藏 的标志
      }
    },
    mounted(){
      ...
      sa.track('learning_click', {
        'click_id': 'pc_learning_video_record_open_record_book'+ this.bookId
      }),
      //所有绘本打开的次数
      sa.track('learning_click', {
        'click_id': 'pc_learning_video_record_open_all_record_book'
      })
    },
    watch:{
    ...
    methods: {
      ...
      monitorAudio() {
        let audio = document.querySelectorAll('audio.parentaduio')
      ...
      audioControlOnPageCorner(event) {
        if (!this.enableAction) return // jcy add: book_record_aciton 组件的操作在进行中是，不允许 播放原音频
      ...
      // added by jcy: 当处在 录音播放 或者 录制音频播放状态时，控制按钮的显示隐藏
      coordinateTurnBtns(bool) {
    ...
    .book-contain
      ...
      .reader-content
        ...
        .sound-control-btn
          ...
          &.disabled
            opacity: 0.7
            box-shadow: 0 0 44px 44px rgba(0, 0, 0, 0.5) inset
            border-radius: 44px
        .replay-btn-right
